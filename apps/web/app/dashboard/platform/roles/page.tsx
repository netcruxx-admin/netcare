...
// Codes the app itself depends on (schemas.Role / dashboard routes). The
// backend refuses to delete these; the UI mirrors that so the button is honest.
const BUILTIN_CODES = new Set(['superadmin', 'admin', 'doctor', 'nurse', 'lab', 'patient']);
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {['Code', 'Display Name', 'Description', 'Scope', 'Users', ''].map((h) => (
              ...
              <tbody>
                {roles.map((r) => {
                  const builtin = BUILTIN_CODES.has(r.code);
                  ...
                  return (
                    <tr key={r.code} className="border-b border-slate-50 hover:bg-slate-50 transition">
                      ...
                      <td className="py-3 px-6 font-medium text-slate-900">{r.label}</td>
                      <td className="py-3 px-6 text-slate-600 text-sm">{r.description || '—'}</td>
